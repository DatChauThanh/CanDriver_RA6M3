...
#define CAN_RECEIVED_MAILBOX_NOT_FOUND (0x1)
 ...
 **********************************************************************************************************************/
static void can_internal_transmition_mode(  can_ctrl_ins_t* p_ctrl,                       \
                                            const can_operation_mode_t can_operation_mode,\
                                            const can_test_mode_t can_test_mode);

static void can_internal_switch_mode(can_ctrl_ins_t* p_ctrl,const can_operation_mode_t operation_mode);

static void can_internal_mailboxs_config(can_ctrl_ins_t* p_ctrl, can_cfg_t const* const p_cfg);

static void can_interal_fifo_support(can_ctrl_ins_t* p_ctrl, can_cfg_t const* const p_cfg);

static void can_internal_single_mailbox_ID_set(can_cfg_t const* const p_cfg,
                                               volatile uint32_t * p_MB_id,
                                               can_mailbox_t * p_MB_data);

static void can_internal_mailbox_read (can_ctrl_ins_t * p_ctrl, uint32_t mailbox, can_frame_t * p_frame);

static void can_internal_call_callback (can_ctrl_ins_t * p_ctrl, can_callback_arg_t * p_args);

static fsp_err_t can_internal_hw_pin_config();
/***********************************************************************************************************************
 * ISR prototypes
 **********************************************************************************************************************/
...
fsp_err_t HAL_CAN_Open(can_ctrl_t * const p_ctrl_arg, can_cfg_t const * const p_cfg)
{
    ...
    if (p_cfg->can_mailbox_mode == CAN_MAILBOX_FIFO_MODE)
    {
        can_interal_fifo_support(p_ctrl, p_cfg);
    }
...
fsp_err_t HAL_CAN_Read(can_ctrl_t * const p_ctrl_arg, can_frame_t * const p_frame)
{
    ...
    if (p_reg->MSSR_b.SEST == CAN_RECEIVED_MAILBOX_NOT_FOUND)
    {
...
static void can_internal_single_mailbox_ID_set(can_cfg_t const* const p_cfg, volatile uint32_t * p_MB_id ,can_mailbox_t * p_MB_data)
{ 
    can_id_mode_t global_id_mode = p_cfg->p_can_cfg_ctrl->can_g_id_mode;
    uint32_t local_id = p_MB_data->can_mailbox_id;
    if(global_id_mode == CAN_STANDARD_ID_MODE || 
    ((global_id_mode == CAN_MIXED_ID_MODE) && (p_MB_data->can_id_mode == CAN_STANDARD_ID_MODE)) )
    {
        local_id = (local_id << R_CAN0_MB_ID_SID_Pos) & R_CAN0_MB_ID_SID_Msk;
    }
    if(global_id_mode == CAN_EXTENDED_ID_MODE || 
    ((global_id_mode == CAN_MIXED_ID_MODE) && (p_MB_data->can_id_mode == CAN_EXTENDED_ID_MODE)) )
    {
...
static void can_internal_call_callback (can_ctrl_ins_t * p_ctrl, can_callback_arg_t * p_args)
{
    can_callback_arg_t Local_arg = {0};
...
{
    fsp_err_t err;
    err = R_IOPORT_PinsCfg(&g_ioport_ctrl, &g_bsp_can_pin_cfg);
    return err;
}
